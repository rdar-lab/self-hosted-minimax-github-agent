...
- When triggered on an issue in GitHub Actions (CI=true): analyze the issue, create a branch, push IMPLEMENTATION_PLAN.md, then use mcp__github__create_pull_request to open a draft PR with the implementation plan in the description and @minimax in the PR body. Do not implement anything yet.

## GitHub PR Workflow
- When triggered on a PR opened event where @minimax appears in the PR body: implement the plan, committing frequently.
- When triggered on a PR comment containing @minimax: follow the instructions in that comment only.
